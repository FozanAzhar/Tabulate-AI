...
public partial class SettingsViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private List<AppNotificationItem> _recentAlerts = [];

    [ObservableProperty]
    private bool _hasRecentAlerts;
        ...
        ILocalNotificationService localNotifications)
    {
        ...
        _inAppNotifications.NotificationsChanged += (_, _) => RefreshAlerts();
    }
    ...
    private void RefreshAlerts()
    {
        ...
        try
        {
            RecentAlerts = _inAppNotifications.GetRecent(10).ToList();
            HasRecentAlerts = RecentAlerts.Count > 0;
            _inAppNotifications.MarkAllRead();
        }
        catch (Exception ex)
        {
            App.WriteCrashLog($"SettingsViewModel.RefreshAlerts: {ex}");
        }
        finally
        {
    ...
    public async Task SetBudgetAlertsEnabledAsync(bool enabled)
    {
        _appSettings.BudgetAlertsEnabled = enabled;
        BudgetAlertsEnabled = enabled;

        if (enabled)
        {
            // Ask for POST_NOTIFICATIONS before the first real alert.
            await _localNotifications.EnsurePermissionAsync();
    ...
    private async Task SaveBudgetsAsync()
    {
        ...
        // If existing spending already exceeds a newly saved budget, alert now.
        await _budgetAlertService.CheckAndNotifyAsync(showInAppAlert: true);
    }
    ...
    private void SyncFromSettings()
    {
        ...
        BudgetAlertsEnabled = _appSettings.BudgetAlertsEnabled;
